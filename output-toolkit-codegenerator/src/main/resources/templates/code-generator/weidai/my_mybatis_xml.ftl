<!-- fileName: ${classInfo.className}${mapperSuffix} -->
...
<mapper namespace="${mapperPackageName}.${classInfo.className}${mapperSuffix}">
    ...
    <update id="update" parameterType="${classInfo.dtoPackageName}">
        ...
        <set>
            ...
            <if test=" ${fieldItem.fieldName}!= null  <#if fieldItem.fieldClass ="String">and  ${fieldItem.fieldName} !='' </#if>">${fieldItem.columnName} = ${r"#{"}${fieldItem.fieldName}${r"}"}<#if fieldItem_has_next>,</#if>${r"</if>"}
                </#if>
                </#list>
        </set>
        WHERE id = ${r"#{"}id${r"}"}
